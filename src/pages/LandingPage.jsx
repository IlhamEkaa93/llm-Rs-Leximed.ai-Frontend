...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 overflow-x-hidden font-sans selection:bg-blue-500 selection:text-white">
      ...
      <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 backdrop-blur-md border-b border-slate-200 py-3 shadow-sm' : 'bg-transparent py-5'}`}>
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            ...
            >
              <div className="flex flex-col px-6 py-4 space-y-2 font-bold text-slate-700">
      ...
      <section ref={heroRef} className="relative pt-32 pb-20 md:pt-40 md:pb-24 px-6 overflow-hidden min-h-[95vh] flex items-center">
        ...
        <div className="max-w-7xl mx-auto w-full grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <motion.div initial="hidden" animate="show" variants={staggerContainer} className="text-center lg:text-left z-10 pt-10 lg:pt-0">
            ...
            <motion.h1 variants={fadeUp} className="text-5xl md:text-6xl lg:text-7xl xl:text-[5.5rem] font-black text-slate-900 leading-[1.1] tracking-tight">
              Sistem Medis <br className="hidden md:block"/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-emerald-500">Berbasis AI.</span>
            </motion.h1>
            
            <motion.p variants={fadeUp} className="text-base md:text-lg text-slate-600 mt-6 max-w-xl mx-auto lg:mx-0 leading-relaxed font-medium">
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: 0.5 }} className="hidden lg:block relative h-[500px] xl:h-[600px] w-full mt-10 lg:mt-0">
            ...
            <motion.div animate={{ y: [0, -15, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }} className="absolute top-8 right-4 xl:right-10 w-[300px] xl:w-80 bg-white/80 backdrop-blur-xl p-6 rounded-3xl border border-white/50 shadow-2xl shadow-blue-900/10 z-20">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h4 className="font-bold text-slate-900">Dr. Arya S.</h4>
                  <p className="text-xs text-slate-500 font-medium">Spesialis Penyakit Dalam</p>
                </div>
              </div>
            ...
            <motion.div animate={{ y: [0, 15, 0] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut", delay: 1 }} className="absolute top-56 xl:top-64 left-0 xl:-left-4 w-72 bg-slate-900/95 backdrop-blur-md p-6 rounded-3xl border border-slate-700 shadow-2xl shadow-emerald-900/20 z-30">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 text-[10px] font-black uppercase rounded-md tracking-wider">Processing</span>
              </div>
      ...
      <section ref={fiturRef} className="py-24 md:py-32 px-6 bg-white relative z-20 border-y border-slate-100">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16 md:mb-20">
            <h2 className="text-blue-600 font-black tracking-widest uppercase text-xs md:text-sm mb-3">Kapabilitas Sistem</h2>
            <h3 className="text-3xl md:text-4xl lg:text-5xl font-black text-slate-900 mb-6 tracking-tight">Satu Platform, Lima Modul Profesional</h3>
            <p className="text-slate-500 max-w-2xl mx-auto text-base md:text-lg leading-relaxed">LexiMed dirancang spesifik untuk mengatasi beban administratif setiap departemen di fasilitas kesehatan Anda.</p>
          </div>
          ...
          <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-50px" }} className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            <motion.div variants={fadeUp} className="group p-8 bg-slate-50 rounded-[2rem] md:rounded-[2.5rem] hover:bg-blue-600 transition-colors duration-300 border border-slate-100 hover:border-blue-600">
              <div className="w-14 h-14 md:w-16 md:h-16 bg-white text-blue-600 rounded-2xl flex items-center justify-center shadow-sm mb-6 group-hover:scale-110 transition-transform duration-300">
            ...
            <motion.div variants={fadeUp} className="group p-8 bg-slate-50 rounded-[2rem] md:rounded-[2.5rem] hover:bg-emerald-600 transition-colors duration-300 border border-slate-100 hover:border-emerald-600">
              <div className="w-14 h-14 md:w-16 md:h-16 bg-white text-emerald-600 rounded-2xl flex items-center justify-center shadow-sm mb-6 group-hover:scale-110 transition-transform duration-300">
            ...
            <motion.div variants={fadeUp} className="group p-8 bg-slate-50 rounded-[2rem] md:rounded-[2.5rem] hover:bg-purple-600 transition-colors duration-300 border border-slate-100 hover:border-purple-600">
              <div className="w-14 h-14 md:w-16 md:h-16 bg-white text-purple-600 rounded-2xl flex items-center justify-center shadow-sm mb-6 group-hover:scale-110 transition-transform duration-300">
                <Microscope size={28} className="md:w-8 md:h-8" />
              </div>
              <h4 className="text-lg md:text-xl font-black text-slate-900 group-hover:text-white mb-3">Modul Radiologi</h4>
            ...
            <motion.div variants={fadeUp} className="group p-8 bg-slate-50 rounded-[2rem] md:rounded-[2.5rem] hover:bg-amber-500 transition-colors duration-300 border border-slate-100 hover:border-amber-500">
              <div className="w-14 h-14 md:w-16 md:h-16 bg-white text-amber-500 rounded-2xl flex items-center justify-center shadow-sm mb-6 group-hover:scale-110 transition-transform duration-300">
                <LineChart size={28} className="md:w-8 md:h-8" />
              </div>
              <h4 className="text-lg md:text-xl font-black text-slate-900 group-hover:text-white mb-3">Modul Manajemen</h4>
            ...
            <motion.div variants={fadeUp} className="group p-8 bg-slate-50 rounded-[2rem] md:rounded-[2.5rem] hover:bg-slate-800 transition-colors duration-300 border border-slate-100 hover:border-slate-800 lg:col-span-2">
              <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start md:items-center">
                <div className="w-14 h-14 md:w-16 md:h-16 shrink-0 bg-white text-slate-800 rounded-2xl flex items-center justify-center shadow-sm md:mb-0 group-hover:scale-110 transition-transform duration-300">
                  <Lock size={28} className="md:w-8 md:h-8" />
                </div>
                <div>
                  <h4 className="text-lg md:text-xl font-black text-slate-900 group-hover:text-white mb-2 md:mb-3">Keamanan & Tata Kelola IT (Admin)</h4>
      ...
      {/* ===== CARA KERJA SECTION ===== */}
      <section ref={caraKerjaRef} className="py-24 md:py-32 px-6 bg-slate-950 text-white relative z-20 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto relative">
          <div className="text-center mb-16 md:mb-20">
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-black mb-4">Integritas Human-in-the-Loop</h2>
            <p className="text-slate-400 max-w-2xl mx-auto text-base md:text-lg">AI kami bekerja sangat cepat, namun keputusan akhir tetap berada di tangan profesional medis.</p>
          </div>

          <div className="space-y-12 lg:space-y-0">
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-24">
              <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-50px" }} className="bg-slate-900 p-8 md:p-10 rounded-[2rem] border border-slate-800 relative z-10 text-left lg:text-right lg:mr-auto lg:ml-0 shadow-2xl">
                <div className="w-12 h-12 bg-blue-500 text-white rounded-full flex items-center justify-center font-black text-xl mb-6 lg:ml-auto shadow-lg shadow-blue-500/30">1</div>
                <h3 className="text-xl md:text-2xl font-bold mb-3 text-white">Input Data Mentah</h3>
                <p className="text-slate-400 text-sm md:text-base leading-relaxed">Tenaga medis mengetik keluhan bebas, hasil observasi berantakan, atau *copy-paste* catatan lama ke dalam sistem LexiMed.</p>
              </motion.div>
            ...
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-24 mt-8 lg:-mt-16">
              <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-50px" }} className="bg-slate-900 p-8 md:p-10 rounded-[2rem] border border-slate-800 relative z-10 text-left lg:text-right lg:mr-auto lg:ml-0 shadow-2xl shadow-purple-900/20">
                <div className="w-12 h-12 bg-purple-500 text-white rounded-full flex items-center justify-center font-black text-xl mb-6 lg:ml-auto shadow-lg shadow-purple-500/30">3</div>
                <h3 className="text-xl md:text-2xl font-bold mb-3 text-white">Validasi Dokter/Perawat</h3>
                <p className="text-slate-400 text-sm md:text-base leading-relaxed">Draf hasil AI disajikan ke layar. Tenaga medis berhak mengedit, menambah, atau menolak narasi buatan AI sebelum disimpan.</p>
              </motion.div>

              <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-50px" }} className="bg-slate-900 p-8 md:p-10 rounded-[2rem] border border-slate-800 relative z-10 lg:mt-32 shadow-2xl">
                ...
                <h3 className="text-xl md:text-2xl font-bold mb-3 text-white">Finalisasi & Rekam Log</h3>
                <p className="text-slate-400 text-sm md:text-base leading-relaxed">Data tersimpan permanen ke Database PostgreSQL, dapat diakses kapan saja, dan terenkripsi beserta catatan log *Audit Trail*.</p>
              </motion.div>
            </div>
          </div>
        </div>
      ...
      {/* ===== CTA (Call to Action) ===== */}
      <section className="py-24 md:py-32 px-6 bg-gradient-to-br from-blue-600 to-emerald-600 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[500px] md:w-[800px] h-[500px] md:h-[800px] bg-white/10 rounded-full blur-[80px] md:blur-[100px] -translate-y-1/2 translate-x-1/3"></div>
      ...
      {/* ===== FOOTER ===== */}
      <footer className="bg-slate-950 text-slate-400 py-12 md:py-16 px-6 relative z-20 border-t border-slate-900">
        ...
        <div className="max-w-7xl mx-auto pt-8 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4 text-xs font-medium">
          <p>&copy; 2026 LexiMed.ai &mdash; Inovasi AI Kesehatan Vokasi UNS. All rights reserved.</p>
