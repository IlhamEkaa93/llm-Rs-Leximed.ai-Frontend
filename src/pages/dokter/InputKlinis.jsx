...
import { 
    ...
    User, Mic, MicOff, AlertTriangle, FileText, Activity, Loader2,
...
const InputKlinis = () => {
    ...
    // Status Notifikasi (bisa 'success', 'error', atau null)
    ...
    useEffect(() => {
        ...
        if (savedPatient) {
            try {
                ...
                // Mengambil TTV pasien dengan aman (support field norm maupun no_rm)
        ...
        // Setup Web Speech API (Untuk Voice Note)
    ...
    const handleSaveRawData = async () => {
        ...
        try {
            ...
            // PAYLOAD: Gabungkan TTV terakhir dengan Catatan Dokter
            // PERBAIKAN: FIELD 'source' DIHAPUS KARENA TIDAK ADA DI DATABASE
            const payload = {
                ...
                status: "draft" // Draft = Siap diproses oleh AI
            };
            ...
            setSaveStatus('error');
            setErrorMessage(error.response?.data?.message || error.response?.data?.error || "Gagal menyimpan data ke PostgreSQL. Periksa koneksi backend.");
        } finally {
            setIsSaving(false);
        }
    };
                ...
                <div className="flex items-start gap-5 bg-indigo-50/50 p-6 rounded-[2rem] border border-indigo-100">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-indigo-700 mt-1 font-medium leading-relaxed">
                            Input catatan klinis Anda secara detail. Sistem akan otomatis melampirkan <b>Vital Sign</b> terbaru dari pasien ini jika tersedia. Setelah disimpan, data menjadi basis bagi <b>Llama 3.3 AI</b> untuk menyusun laporan.
                        </p>
                    </div>
                </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <label className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                                <FileText size={18} className="text-blue-500" /> Hasil Pemeriksaan & Instruksi
                            </label>
                            ...
                            >
                                {isListening ? <><Activity size={16} /> Merekam Suara...</> : <><MicOff size={16} /> Aktifkan Voice Note</>}
                            </button>
                        </div>
                        ...
                        <textarea 
                            ...
                            placeholder="Ketik diagnosa, hasil anamnesis, tindakan, atau instruksi resep terapi secara bebas di sini..."
                            value={rawText}
                        ...
                        >
                            {isSaving ? <><Loader2 className="animate-spin" size={24} /> SINKRONISASI DATABASE...</> : <><Save size={24} /> SIMPAN & PROSES AI LLAMA</>}
                        </button>
                    </div>
                </motion.div>
            </div>
